...
export interface Vein {
  main: { d: string; len: number };
  twigs: { d: string; len: number }[];
}
...
/* Arbre dendritique récursif. Renvoie [{d, len, depth, dist}]. */
export function tree(opts: TreeOpts): Seg[] {
  const o = {
    x: 0,
    y: 0,
    angle: -Math.PI / 2,
  ...
  (function rec(
    ...
    for (let i = 0; i < n; i++) {
      ...
      rec(x2, y2, a, length * o.decay * (0.86 + rand() * 0.28), depth - 1, cumDist + length);
    }
...
export function vein(
  ...
  const o = { bow: 0.16, twigs: 2, twigLen: 0.5, seed: 3, ...opts };
  const rand = rng(o.seed);
  const sign = rand() > 0.5 ? 1 : -1;
  const main = curvedPath(x1, y1, x2, y2, o.bow * sign);
  const L = dist(x1, y1, x2, y2);
  const twigs: { d: string; len: number }[] = [];
...
export function mark(seed?: number): Seg[] {
  return tree({
    x: 11,
    y: 21,
    angle: -Math.PI / 2,
...
export const Fractal = { rng, tree, vein, mark, curvedPath };
